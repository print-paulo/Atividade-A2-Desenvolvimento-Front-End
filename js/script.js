...
function atualizarCarrinho() {
  ...
  carrinho.forEach((item, index) => {
    ...
    subtotalSpan.textContent = ` Subtotal: R$ ${(
...
function desenharFatiaDePizza() {
  ...
  ctx.arc(
    ...
    raio,
    anguloInicial,
    anguloInicial + Math.PI * 0.3 // Um pouco menos que 45 graus para um visual melhor
  );
...
if (canvas) {
  ...
  document.getElementById("resetBtn").addEventListener("click", () => {
    ...
    document
      .querySelectorAll(".botoes-ingredientes button")
      .forEach((button) => {
  ...
  document.getElementById("addToCartBtn").addEventListener("click", () => {
    ...
    alert(
      `Pizza Personalizada Adicionada à Cesta! Ingredientes: Molho, ${
        ingredientesSelecionados || "Base Simples"
      }.`
    );
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const mostrarResultados = () => {
    ...
    const encontrados = Array.from(itens).filter((item) => {
      ...
      const ingredientes =
        item.querySelector(".item-ingredientes")?.textContent.toLowerCase() ||
        "";
    ...
    encontrados.forEach((item) => {
      ...
      const ingredientes =
        item.querySelector(".item-ingredientes")?.textContent || "";
